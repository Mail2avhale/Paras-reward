...
import { Activity, Gift, Star } from 'lucide-react';

/**
 * Live Rewards Indicator Component (AdMob Compliant)
 * Shows animated "Rewards Active • Live" badge when daily rewards session is active
 * Google Play & AdMob Compliant - Uses loyalty points terminology
 */
const LiveRewardsIndicator = ({ isMining, miningEndTime }) => {
  ...
  if (!isMining) {
    return (
      <div className="flex items-center gap-2 bg-gray-500/20 backdrop-blur-sm px-3 py-1.5 rounded-full border border-gray-400/30">
        ...
        <span className="text-xs text-gray-300 font-medium">No Active Session</span>
      </div>
    );
  }
  ...
  return (
    <div 
      ...
      data-testid="live-rewards-indicator"
    >
      {/* Animated Dot */}
      ...
      <div className="flex items-center gap-1.5">
        <Star className="w-3.5 h-3.5 text-green-300" />
        <span className="text-xs text-green-200 font-semibold">Collecting Points</span>
...
export default LiveRewardsIndicator;
