...
const PaymentRetryQueueWidget = () => {
  ...
  if (loading) {
    return (
      <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-slate-200 p-6" data-testid="retry-queue-loading">
  ...
  return (
    <Card className="bg-gradient-to-br from-gray-900 to-gray-800 border-slate-200 p-6" data-testid="payment-retry-queue-widget">
      {/* Header */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-yellow-500/10 rounded-xl p-4 border border-yellow-500/20" data-testid="pending-count">
          ...
          <p className="text-xs text-slate-500">+ {stats?.retry_scheduled || 0} scheduled</p>
        </div>
        ...
        <div className="bg-red-500/10 rounded-xl p-4 border border-red-500/20" data-testid="failed-count">
          ...
          <p className="text-2xl font-bold text-slate-800">{stats?.failed || 0}</p>
          <p className="text-xs text-slate-500">max retries exceeded</p>
        </div>
        ...
        <div className="bg-blue-500/10 rounded-xl p-4 border border-blue-500/20" data-testid="success-rate">
          ...
          <p className="text-xs text-slate-500">{totalProcessed} total tasks</p>
        </div>
      </div>
      ...
      {retrySettings && (
        <div className="bg-slate-50 rounded-lg p-4 mb-6 border border-slate-200">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-slate-800">Auto-Retry Configuration</span>
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="text-slate-800 font-medium">{retrySettings.max_retries}x</p>
            </div>
            <div>
              <p className="text-slate-500">Initial Delay</p>
              <p className="text-slate-800 font-medium">{retrySettings.retry_delay_seconds}s</p>
            </div>
            <div>
              <p className="text-slate-500">Backoff</p>
              <p className="text-slate-800 font-medium">1m → 5m → 30m</p>
            </div>
          </div>
      ...
      {pendingRetries.length > 0 ? (
        ...
        <div className="text-center py-6 text-slate-500" data-testid="no-pending-retries">
