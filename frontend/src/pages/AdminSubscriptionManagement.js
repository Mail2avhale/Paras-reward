    ...
    <div className="min-h-screen bg-gray-950 p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm">Manage Elite subscriptions</p>
        </div>
        <Button onClick={fetchData} variant="outline" size="sm" className="border-slate-200">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              className="bg-white border-slate-200 text-slate-800 w-40"
            />
          </div>
          ...
          {(dateFrom || dateTo || searchQuery) && (
            <Button
              ...
              className="border-slate-200 text-slate-500 hover:text-slate-800"
            >
        ...
        {showAdvancedFilters && (
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
            ...
            <div className="flex flex-wrap items-end gap-4">
              ...
              <div className="flex-1 min-w-[160px]">
                <label className="block text-xs text-slate-500 mb-1.5">Subscription Type</label>
                <select
            ...
            <div className="flex flex-wrap items-end gap-4">
              <div className="flex-1 min-w-[120px]">
                <label className="block text-xs text-slate-500 mb-1.5">Min Amount (₹)</label>
                <Input
                  type="number"
                  placeholder="0"
                  value={amountMin}
              ...
              <div className="flex-1 min-w-[120px]">
                <label className="block text-xs text-slate-500 mb-1.5">Max Amount (₹)</label>
                <Input
                  type="number"
                  placeholder="10000"
                  value={amountMax}
      ...
      {imageModal.show && imageModal.url && (
        ...
        >
          <button className="absolute top-4 right-4 text-slate-800 p-2 bg-white/10 rounded-full">
            <X className="w-6 h-6" />
          </button>
          <img 
            src={imageModal.url} 
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-4">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${colors[color]} flex items-center justify-center text-slate-800`}>
      ...
      <div className="flex flex-col md:flex-row gap-4">
          ...
          {payment.screenshot_url ? (
            <button onClick={onImageClick} className="w-full group relative">
              <img 
                ...
                alt="Screenshot" 
                className="w-full h-32 object-cover rounded-lg border border-slate-200 group-hover:border-purple-500"
              />
            ...
            <div className="w-full h-32 bg-white rounded-lg border border-dashed border-slate-200 flex items-center justify-center">
        ...
        <div className="flex-1">
          <div className="flex items-start justify-between mb-2">
            <div>
              <h3 className="text-slate-800 font-bold">{payment.user_name || 'Unknown'}</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-3">
            ...
            <div className="bg-slate-50 rounded-lg p-2">
              ...
              <p className="text-slate-800 capitalize">{payment.duration || payment.plan_type || 'monthly'}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-2">
              <p className="text-slate-500 text-xs">UTR</p>
              <p className="text-slate-800 font-mono text-sm truncate">{payment.utr_number || 'N/A'}</p>
            </div>
            <div className="bg-slate-50 rounded-lg p-2">
              <p className="text-slate-500 text-xs">Submit Date</p>
            ...
            {tab === 'pending' && payment.current_plan && (
              <div className="bg-slate-50 rounded-lg p-2">
                ...
                <p className={`text-sm font-medium ${
                  payment.current_plan === 'explorer' ? 'text-slate-500' : 'text-purple-400'
                }`}>
  ...
  <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
    <div className="bg-white rounded-2xl border border-slate-200 p-6 max-w-lg w-full">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div><p className="text-slate-500">User</p><p className="text-slate-800">{payment.user_name}</p></div>
        <div><p className="text-slate-500">Email</p><p className="text-slate-800">{payment.user_email}</p></div>
        <div><p className="text-slate-500">Plan</p><p className="text-slate-800 capitalize">{payment.plan || payment.subscription_plan}</p></div>
        <div><p className="text-slate-500">Amount</p><p className="text-slate-800">₹{payment.amount}</p></div>
        <div><p className="text-slate-500">Duration</p><p className="text-slate-800 capitalize">{payment.duration || payment.plan_type}</p></div>
        <div><p className="text-slate-500">UTR</p><p className="text-slate-800 font-mono">{payment.utr_number || 'N/A'}</p></div>
        <div><p className="text-slate-500">Expiry</p><p className="text-slate-800">{payment.new_expiry ? new Date(payment.new_expiry).toLocaleDateString() : 'N/A'}</p></div>
        <div><p className="text-slate-500">Status</p><p className="text-slate-800 capitalize">{payment.status}</p></div>
      </div>

      <Button onClick={onClose} className="w-full mt-6 bg-white hover:bg-slate-100">Close</Button>
    </div>
  </div>
);
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl border border-slate-200 p-6 max-w-md w-full">
        ...
        <div className="space-y-4">
          <div>
            <label className="text-slate-500 text-sm block mb-1">Plan</label>
            <select
              value={form.plan}
          ...
          <div>
            <label className="text-slate-500 text-sm block mb-1">Duration</label>
            <select
              value={form.duration}
          ...
          <div>
            <label className="text-slate-500 text-sm block mb-1">Amount (₹)</label>
            <Input
              type="number"
              value={form.amount}
          ...
          <div>
            <label className="text-slate-500 text-sm block mb-1">Expiry Date</label>
            <Input
              type="date"
        ...
        <div className="flex gap-3 mt-6">
          <Button onClick={onClose} variant="outline" className="flex-1 border-slate-200">Cancel</Button>
          <Button 
            onClick={() => onSave(form)} 
            disabled={processing}
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl border border-slate-200 p-6 max-w-md w-full">
        ...
        <div className="mb-4 p-3 bg-white rounded-lg">
          <p className="text-slate-800 font-medium">{payment.user_name}</p>
          <p className="text-slate-500 text-sm">₹{payment.amount} - {payment.plan?.toUpperCase()}</p>
        </div>
        ...
        <Input
          ...
          className="bg-white border-slate-200 text-slate-800 mb-4"
        />
  ...
  return (
    <div className="bg-white rounded-xl p-4 mb-6 border border-slate-200">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-lg font-semibold text-slate-800">Edit User Subscription</h2>
      </div>
        ...
        <div className="bg-white rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4 pb-4 border-b border-slate-200">
            ...
            <div>
              ...
              <p className="text-slate-800">{user.mobile || 'N/A'}</p>
            </div>
            <div>
              <p className="text-slate-500 text-xs">Current Plan</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Plan Selection */}
            <div>
              <label className="text-slate-500 text-sm mb-2 block">New Plan</label>
              <select
                value={plan}
            ...
            {/* Expiry Mode */}
            <div>
              <label className="text-slate-500 text-sm mb-2 block">Expiry Mode</label>
              <div className="flex gap-2">
                <button
                  onClick={() => setExpiryMode('days')}
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium ${
                    expiryMode === 'days' ? 'bg-purple-600 text-slate-800' : 'bg-white text-slate-500 border border-slate-200'
                  }`}
                >
                  Add Days
                </button>
                <button
                  ...
                  className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium ${
                    expiryMode === 'date' ? 'bg-purple-600 text-slate-800' : 'bg-white text-slate-500 border border-slate-200'
                  }`}
                >
                  Set Date
                </button>
            ...
            {expiryMode === 'days' ? (
              <div>
                <label className="text-slate-500 text-sm mb-2 block">Days to Add</label>
                <Input
                  type="number"
                  value={days}
              ...
              <div>
                <label className="text-slate-500 text-sm mb-2 block">Expiry Date</label>
                <Input
                  type="date"
                  value={expiryDate}
            ...
            {/* Notes */}
            <div>
              <label className="text-slate-500 text-sm mb-2 block">Admin Notes</label>
              <Input
