    ...
    <div className="space-y-6 p-4 md:p-6 pb-24">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            className={autoRefresh ? 'bg-emerald-600 hover:bg-emerald-700' : 'border-slate-200'}
          >
          ...
          {['today', 'week', 'month', 'year'].map(range => (
            <Button
              ...
              className={dateRange === range ? 'bg-purple-600' : 'border-slate-200'}
            >
      ...
      <Card className="bg-white border-slate-200 p-4 mb-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <span className="text-slate-500 text-sm">Custom Range:</span>
          <Input
            type="date"
            value={customStart}
            onChange={(e) => setCustomStart(e.target.value)}
            className="w-40 bg-white border-slate-200 text-slate-800"
          />
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 p-4">
            ...
            <p className="text-2xl font-bold text-slate-800">{formatPRC(prc.total_in_wallets || 0)}</p>
            <p className="text-xs text-slate-500 mt-1">Total in User Wallets</p>
          ...
          <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/30 p-4">
            ...
            <p className="text-2xl font-bold text-slate-800">{formatPRC(prc.total_mined || 0)}</p>
            <p className="text-xs text-slate-500 mt-1">Total PRC Mined</p>
          ...
          <Card className="bg-gradient-to-br from-purple-500/10 to-purple-600/5 border-purple-500/30 p-4">
            ...
            <p className="text-2xl font-bold text-slate-800">{formatPRC(prc.total_circulated || 0)}</p>
            <p className="text-xs text-slate-500 mt-1">Total Circulated</p>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          {[
            ...
            return (
              <Card key={period.key} className="bg-white border-slate-200 p-4">
                <p className={`text-xs text-${period.color}-400 mb-2 font-medium`}>{period.label}</p>
                <p className="text-xl font-bold text-slate-800">{formatPRC(stats.total_prc || 0)}</p>
                <p className="text-sm text-slate-500 mt-1">{formatINR(stats.prc_value_inr || 0)}</p>
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-200">
                  <span className="text-xs text-slate-500">{stats.total_count || 0} redemptions</span>
                </div>
              </Card>
            );
          })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="bg-white border-slate-200 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-slate-800 font-medium">Shopping Orders</p>
                <p className="text-xs text-slate-500">Product redemptions</p>
              </div>
            </div>
            <p className="text-2xl font-bold text-slate-800">{formatPRC(redemption.all_time?.breakdown?.orders?.prc || 0)}</p>
            <p className="text-sm text-slate-500 mt-1">{redemption.all_time?.breakdown?.orders?.count || 0} orders</p>
          </Card>
          ...
          <Card className="bg-white border-slate-200 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-slate-800 font-medium">Bill Payments</p>
                <p className="text-xs text-slate-500">Recharges & bills</p>
              </div>
            </div>
            <p className="text-2xl font-bold text-slate-800">{formatPRC(redemption.all_time?.breakdown?.bill_payments?.prc || 0)}</p>
            <p className="text-sm text-emerald-400 mt-1">{formatINR(redemption.all_time?.breakdown?.bill_payments?.inr || 0)} INR Value</p>
            <p className="text-sm text-slate-500">{redemption.all_time?.breakdown?.bill_payments?.count || 0} payments</p>
          </Card>
          ...
          <Card className="bg-white border-slate-200 p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <p className="text-slate-800 font-medium">Gift Vouchers</p>
                <p className="text-xs text-slate-500">Voucher redemptions</p>
              </div>
            </div>
            <p className="text-2xl font-bold text-slate-800">{formatPRC(redemption.all_time?.breakdown?.gift_vouchers?.prc || 0)}</p>
            <p className="text-sm text-purple-400 mt-1">{formatINR(redemption.all_time?.breakdown?.gift_vouchers?.inr || 0)} INR Value</p>
            <p className="text-sm text-slate-500">{redemption.all_time?.breakdown?.gift_vouchers?.count || 0} vouchers</p>
          </Card>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <Card className="bg-gradient-to-br from-blue-500/10 to-blue-600/5 border-blue-500/30 p-4">
            ...
            <p className="text-3xl font-bold text-slate-800">{users.total || 0}</p>
          ...
          <Card className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border-emerald-500/30 p-4">
            ...
            <p className="text-3xl font-bold text-slate-800">{users.active || 0}</p>
          ...
          <Card className="bg-gradient-to-br from-red-500/10 to-red-600/5 border-red-500/30 p-4">
            ...
            <p className="text-3xl font-bold text-slate-800">{users.inactive || 0}</p>
          ...
          <Card className="bg-gradient-to-br from-amber-500/10 to-amber-600/5 border-amber-500/30 p-4">
            ...
            <p className="text-3xl font-bold text-slate-800">{users.paid_users || 0}</p>
            <p className="text-xs text-slate-500 mt-1">Paid Subscribers</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="bg-white border-slate-200 p-4">
            <h3 className="text-slate-800 font-medium mb-4">Plan Distribution</h3>
            <div className="h-64">
          ...
          <Card className="bg-white border-slate-200 p-4">
            <h3 className="text-slate-800 font-medium mb-4">User Activity Status</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          {[
            ...
            return (
              <Card key={plan.name} className="bg-slate-50 border-slate-200 p-3">
                <div className="flex items-center gap-2 mb-2">
                  <Icon className={`w-4 h-4 text-${plan.color}-400`} />
                  <span className="text-sm text-slate-500">{plan.name}</span>
                </div>
                <p className="text-2xl font-bold text-slate-800">{plan.value}</p>
              </Card>
            );
          })}
        </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* User Growth Chart */}
          <Card className="bg-white border-slate-200 p-4">
            <h3 className="text-slate-800 font-medium mb-4">User Growth (Last 30 Days)</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <Card className="bg-white border-slate-200 p-4">
            <h3 className="text-slate-800 font-medium mb-4">Redemption Trend (Last 30 Days)</h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-200 bg-white">
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-500">#</th>
                  ...
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-500">Balance</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-500">INR Value</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-500">KYC</th>
                  <th className="text-left py-3 px-4 text-sm font-semibold text-slate-500">Action</th>
                </tr>
              </thead>
              <tbody>
                {(data?.top_users || []).map((topUser, index) => (
                  <tr key={topUser.uid} className="border-b border-slate-200/50 hover:bg-slate-50">
                    ...
                    <td className="py-3 px-4">
                      <p className="text-slate-800 font-medium">{topUser.name || 'Unknown'}</p>
                      <p className="text-xs text-slate-500">{topUser.email}</p>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <p className="text-slate-800">{formatINR((topUser.prc_balance || 0) * 0.1)}</p>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded text-xs ${
                    ...
                    <td className="py-3 px-4">
                      <Button
                        size="sm"
                        variant="outline"
                        className="border-slate-200 h-7"
