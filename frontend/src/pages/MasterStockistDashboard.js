...
import { Package, TrendingUp, Users, DollarSign, Truck, BarChart3, Search } from 'lucide-react';
...
const MasterStockistDashboard = ({ user, onLogout }) => {
  ...
  const fetchDashboardData = async () => {
    try {
      // Fetch stats
      const statsRes = await axios.get(`${API}/master-stockist/${user.uid}/stats`);
      setStats(statsRes.data);

      // Fetch sub stockists
      const subRes = await axios.get(`${API}/master-stockist/${user.uid}/sub-stockists`);
      setSubStockists(subRes.data);

      // Fetch inventory
      const invRes = await axios.get(`${API}/master-stockist/${user.uid}/inventory`);
      setInventory(invRes.data);

      // Fetch orders
      const ordersRes = await axios.get(`${API}/master-stockist/${user.uid}/orders`);
      setOrders(ordersRes.data);

      // Fetch wallet
      const walletRes = await axios.get(`${API}/master-stockist/${user.uid}/wallet`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage your inventory, sub-stockists, and orders</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white p-6 rounded-2xl shadow-xl">
            ...
            <div className="text-purple-100">Total Stock Units</div>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl shadow-xl">
            ...
            <div className="text-blue-100">Profit Wallet</div>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-pink-500 to-pink-600 text-white p-6 rounded-2xl shadow-xl">
            ...
            <div className="text-3xl font-bold mb-1">{stats.activeSubStockists || 0}</div>
            <div className="text-pink-100">Sub Stockists</div>
          </Card>
        ...
        <Card className="bg-white/80 backdrop-blur-sm p-8 rounded-3xl shadow-xl">
          <Tabs defaultValue="inventory" className="w-full">
            <TabsList className="grid w-full grid-cols-4 mb-6">
              ...
              <TabsTrigger value="substockists">Sub Stockists</TabsTrigger>
            ...
            <TabsContent value="inventory" className="space-y-4">
              ...
              {inventory.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400">Stock will appear here once allocated by company</p>
                </div>
              ) : (
            ...
            {/* SUB STOCKISTS TAB */}
            <TabsContent value="substockists" className="space-y-4">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Sub Stockists Network</h2>

              {subStockists.length === 0 ? (
                <div className="text-center py-12">
                  <Users className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                  <p className="text-gray-500">No sub stockists yet</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {subStockists.map((sub) => (
                    <Card key={sub.id} className="p-6 hover:shadow-xl transition-all">
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="font-bold text-gray-900">{sub.business_name}</h3>
                          <p className="text-sm text-gray-600">{sub.owner_name}</p>
                        </div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{sub.total_stock || 0}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Sales:</span>
                          <span className="font-medium">₹{sub.total_sales?.toLocaleString() || 0}</span>
                        </div>
                        <div className="flex justify-between">
            ...
            <TabsContent value="reports" className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card className="p-6">
                  ...
                  <h3 className="text-xl font-bold text-gray-900 mb-2">Network Report</h3>
                  <p className="text-gray-600 mb-4">Sub-stockist performance overview</p>
                  <Button className="w-full">Generate Report</Button>
                </Card>
              </div>
            </TabsContent>
