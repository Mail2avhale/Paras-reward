...
import {
  Package, Search, Plus, Edit, Trash2, RefreshCw, X, Upload, Eye, EyeOff,
  Tag, DollarSign, Box, Image as ImageIcon, Percent, Truck, Star, Home,
  TrendingUp, AlertCircle, CheckCircle, IndianRupee, Save
} from 'lucide-react';
...
// PRC to INR conversion rate (configurable)
...
const STOCK_STATUS = [
  ...
  { value: 'limited', label: 'Limited Stock', color: 'orange' },
...
const AdminMarketplace = ({ user }) => {
  ...
  const calculateMargin = (cost, prcPrice) => {
    const inrPrice = parseFloat(prcPrice) * prcToInrRate;
    const costValue = parseFloat(cost) || 0;
    if (costValue <= 0 || inrPrice <= 0) return 0;
    return ((inrPrice - costValue) / inrPrice * 100).toFixed(2);
  };

  const handleCostChange = (value) => {
    const margin = calculateMargin(value, formData.prc_price);
    setFormData({
      ...formData,
          ...
          <div>
            ...
            <div className="relative">
              <Input
                ...
                onChange={(e) => setFormData({...formData, inr_price: e.target.value})}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({...formData, delivery_charge_type: e.target.value, delivery_charge_value: e.target.value === 'free' ? '0' : formData.delivery_charge_value})}
            >
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => setFormData({...formData, delivery_charge_value: e.target.value})}
                  className="bg-gray-800 border-gray-700 text-white pr-8"
                />
                ...
                <div className="p-4">
                  ...
                  {product.margin_percent > 0 && (
                    <div className="text-xs text-gray-500 mb-3">
                      Margin: <span className={`font-semibold ${product.margin_percent > 20 ? 'text-green-400' : 'text-yellow-400'}`}>{product.margin_percent?.toFixed(1)}%</span>
                    </div>
                  )}
