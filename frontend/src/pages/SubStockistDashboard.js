...
import { Package, DollarSign, Truck, AlertCircle, CheckCircle, Store } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Manage outlets, inventory, and earnings</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <Card className={`bg-gradient-to-br ${renewalStatus?.is_overdue ? 'from-red-500 to-red-600' : 'from-teal-500 to-teal-600'} text-white p-6 rounded-2xl shadow-xl`}>
            ...
            <div className="text-white opacity-90">Renewal</div>
          </Card>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            {securityDeposit ? (
              <div>
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  <p>Next Return: <span className="font-semibold">{securityDeposit.days_until_next_return || 'N/A'} days</span></p>
                </div>
              </div>
            ) : (
          ...
          <Card className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl shadow-xl">
            ...
            {renewalStatus ? (
              <div className="text-sm text-gray-600 space-y-1">
                <p>Due: <span className="font-semibold">{renewalStatus.renewal_due_date ? new Date(renewalStatus.renewal_due_date).toLocaleDateString() : 'N/A'}</span></p>
                <p>Days Left: <span className="font-semibold">{renewalStatus.days_until_due || 'N/A'}</span></p>
                {renewalStatus.is_overdue && (
                  <p className="text-red-600 font-semibold">⚠️ Overdue</p>
                )}
              </div>
            ) : (
              <p className="text-gray-500">Fee: ₹30,000 + GST</p>
            )}
          </Card>
        </div>
          ...
          <Tabs defaultValue="stock" className="w-full">
            ...
            <TabsContent value="stock">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">Sent to Outlets</h3>
                  {stockMovements.sent?.length > 0 ? (
                      ...
                      {stockMovements.sent.map((movement, idx) => (
                        <Card key={idx} className="p-4 border-l-4 border-purple-500">
                          <div className="flex justify-between">
                            <div>
                              <p className="font-semibold">{movement.product_name || 'Transfer'}</p>
                              <p className="text-sm text-gray-600">Qty: {movement.quantity}</p>
                              <p className="text-xs text-gray-500">Batch: {movement.batch_number}</p>
                            </div>
                            <span className={`px-3 py-1 rounded-full text-xs h-fit ${
                              movement.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                            }`}>
                              {movement.status}
                            </span>
                          </div>
                    ...
                    <p className="text-gray-500 text-center py-8">No transfers</p>
                  )}
                </div>

                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">Received from Master</h3>
            ...
            <TabsContent value="outlets">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Outlet Management</h2>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
                <Store className="h-12 w-12 text-blue-500 mx-auto mb-3" />
                <p className="text-gray-700">Contact admin to allocate stock to outlets</p>
              </div>
            </TabsContent>
